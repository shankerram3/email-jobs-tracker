...
def upgrade() -> None:
    ...
    # Ensure expected indexes exist (safe for repeated runs).
    existing_indexes = set()
    try:
        for ix in inspector.get_indexes("resumes"):
            if ix and ix.get("name"):
                existing_indexes.add(ix["name"])
    except Exception:
        # Some SQLite setups may not return indexes reliably; fall back to best-effort creates below.
        existing_indexes = set()

    def _create_index_if_missing(name: str, cols: list[str]):
        if existing_indexes and name in existing_indexes:
            return
        try:
            op.create_index(name, "resumes", cols)
        except Exception:
            # If it already exists (or SQLite doesn't support an exact op), ignore.
            pass
    ...
    op.execute(
        ...
        SET application_stage = 'Offer',
            requires_action = 1,
            action_items = '["Review offer details and respond"]'
